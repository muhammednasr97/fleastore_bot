...
from bot.fsm_telebot.storage.base import BaseStorage, DisabledStorage
...
class TeleBot(telebot.TeleBot):
    ...
    def callback_query_handler(self, func, state='*', **kwargs):
        def decorator(handler):
            handler_dict = self._build_handler_dict(handler, func=func, state=state, **kwargs)
    ...
    def _test_filter(self, filter, filter_value, message):
        test_cases = {
            'state': lambda msg: True if filter_value == '*' else
            self.storage.get_state(msg.chat.id, msg.from_user.id, default='') == filter_value
    ...
    def get_data(self, chat_id=None, user_id=None, default=None):
        ...
        :return:
        """
        return self.storage.get_data(chat_id, user_id, default=default)
